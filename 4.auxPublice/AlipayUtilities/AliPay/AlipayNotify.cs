...
namespace AlipayUtilities.AliPay
{
    ...
    public class Notify
    {
        ...
        public bool Verify(SortedDictionary<string, string> inputPara, string notify_id, string sign, string signType)
        {

            //File.AppendAllText(HttpContext.Current.Server.MapPath("/log/notify.txt"), "进入，验证消息是否是支付宝发出的合法消息 \r\n");


            //获取返回时的签名验证结果
            bool isSign = GetSignVeryfy(inputPara, sign, signType);
            //获取是否是支付宝服务器发来的请求的验证结果
            string responseTxt = "false";
            if (notify_id != null && notify_id != "") { responseTxt = GetResponseTxt(notify_id); }
            //File.AppendAllText(HttpContext.Current.Server.MapPath("/log/notify.txt"), "获取是否是支付宝服务器发来的请求的验证结果:" + responseTxt + " | " + notify_id + "\r\n");
            ...
            {
                //File.AppendAllText(HttpContext.Current.Server.MapPath("/log/notify.txt"), "验证成功 \r\n");

                return true;
            }
            else//验证失败
            {
                //File.AppendAllText(HttpContext.Current.Server.MapPath("/log/notify.txt"), "验证失败 \r\n");
                return false;
            }
        }
        ...
        private bool GetSignVeryfy(SortedDictionary<string, string> inputPara, string sign, string signType)
        {
            Dictionary<string, string> sPara = new Dictionary<string, string>();
