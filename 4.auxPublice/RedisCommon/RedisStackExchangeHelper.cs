...
namespace RedisCommon
{
    ...
    public class RedisStackExchangeHelper
    {
        ...
        /// <param name="key">Redis Key</param>
        /// <returns></returns>
        public string StringGet(string key, string SysCustomKey = "")
        {
            if (SysCustomKey != "")
            {
                key = SysCustomKey + key;
        ...
        public object HashKeysValues<T>(string key)
        {
            key = AddSysCustomKey(key);
            return Do(db =>
            {
                ...
                return ConvetListTo<T>(values);
            });
        }

        public int HashKeysCount(string key)
        {
            key = AddSysCustomKey(key);
            return Do(db =>
            {
                return db.HashGetAll(key).Count();
            });
        }
        ...
        private List<T> ConvetListTo<T>(HashEntry[] values)
        {
