...
namespace SqlSugar
{
    public abstract partial class DbFirstProvider : IDbFirst
    {
        ...
        #region Core
        public class ToClassMolde
        {
            public string classText { get; set; }
            public string className { get; set; }
            public string classType { get; set; }
        }
        public List<ToClassMolde> ToClassStringList(string nameSpace = "Models")
        {
            this.Namespace = nameSpace;
            List<ToClassMolde> result = new List<ToClassMolde>();
            if (this.TableInfoList.HasValue())
            {
                foreach (var tableInfo in this.TableInfoList)
                {
                    ToClassMolde model = new ToClassMolde();
                    string classText = null;
                    string className = tableInfo.Name;
                    string classType = tableInfo.DbObjectType.ToString();
                    classText = GetClassString(tableInfo, ref className);

                    model.className = className;
                    model.classText = classText;
        ...
        internal string GetClassString(DbTableInfo tableInfo, ref string className)
        {
            ...
            if(tableInfo.DbObjectType.ToString() == "View")
                classText = classText.Replace(DbFirstTemplate.KeyNamespace, this.Namespace+".View");
            else
            ...
            if (columns.HasValue())
            {
                foreach (var item in columns)
                {
                    ...
                    PropertyDescriptionText += "\n           [System.ComponentModel.Description(\"" + item.ColumnDescription + "\")]";
                    ...
                    {
                        PropertyText = PropertyText.Replace("DateTime?", "byte []");
                    }
        ...
        public void CreateClassFile(string directoryPath, string nameSpace = "Models")
        {
            ...
            if (classStringList.Count > 0)
            {
                foreach (var item in classStringList)
                {
                    if (item.classType == "View")
                    {
                        var di = (directoryPath + @"\View\");
                        FileHelper.CreateFile(di.TrimEnd('\\').TrimEnd('/') + string.Format("\\{0}.cs", item.className), item.classText, Encoding.UTF8);
                    }
                    else
                        FileHelper.CreateFile(directoryPath.TrimEnd('\\').TrimEnd('/') + string.Format("\\{0}.cs", item.className), item.classText, Encoding.UTF8);
                }
            }
        }
        #endregion
